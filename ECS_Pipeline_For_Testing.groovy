            ...
            steps {
                ...
                script {
                    ...
                    if (stackExists == 0) {
                        script {
                            sh '''
                            aws cloudformation update-stack --stack-name ${Stack_Name} --capabilities CAPABILITY_NAMED_IAM  --parameters ParameterKey=ImageId,ParameterValue=${AWS_Account_Id}.dkr.ecr.${Region_Name}.amazonaws.com/${ECR_Repo_Name}:${Version_Number} ParameterKey=ContainerPort,ParameterValue=${Container_Port} || true
                           '''
                        }
                    } else {
                        script {
